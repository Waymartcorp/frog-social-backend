...
export async function generateDraftDelta(
  ...
  // 1. Construct the prompt for GPT-4
  // We send the current summary + the new message to save tokens, 
  // or you could send the whole history if you have it. 
  // For this MVP, let's assume we send the latest context.
  ...
  try {
    ...
    // Return the format our app expects (A "Delta" update)
    return {
      draft_revision: currentDraft.revision + 1,
      ...parsed, // This spreads summary, extracted, and highlights into the object
    };
